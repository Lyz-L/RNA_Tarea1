...
class Network(object): 
    ...
    def __init__(self, sizes): # Número de neuronas en las capas de la red.
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None): #Parámetros de la función SGD
        training_data = list(training_data) # Representa las entradas y salidas deseadas
        n = len(training_data)
    ...
    def update_mini_batch(self, mini_batch, eta): # Actualiza los pesos y los biases
        ...
        self.momentum_v = [ (momentum_mu * self.momentum_v) - ( ( float(eta) / float(len(mini_batch)) )* nw) # Definición del momento
                       for nw in nabla_w ] # Actualización de los pesos
